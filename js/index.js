...
document.addEventListener("load", addData())

function addData() {
	const trainersCardContainer = document.querySelector(".trainers-cards__container");
	const trainerTemp = document.querySelector("#trainer-card");
	...
	function trainersAppend(array) {
		array.forEach((trainer) => {
			const trainerCard = trainerTemp.content.cloneNode(true);
				
			trainerCard.querySelector(".trainer__name").textContent = trainer["first name"];
			trainerCard.querySelector(".trainer__img").setAttribute("src", trainer.photo);
		
			trainersCardContainer.append(trainerCard);
			});
	};
	...
	trainersCardContainer.addEventListener('click', (event) => {
        if (event.target && event.target.classList.contains('trainer__show-more')) {
			document.body.style.overflow = 'hidden';
            const listItem = event.target.closest('.trainer');

			const foundElem = DATA.find(item => item.photo === listItem.querySelector(".trainer__img").getAttribute("src"));

			const modal = modalTemp.content.cloneNode(true);
			modal.querySelector(".modal__img").setAttribute("src", foundElem.photo);
			modal.querySelector(".modal__name").textContent = foundElem["first name"] + " " + foundElem["last name"];
			modal.querySelector(".modal__point.modal__point--category").textContent = "Категорія: " + foundElem.category;
			modal.querySelector(".modal__point.modal__point--experience").textContent = "Досвід: " + foundElem.experience;
			modal.querySelector(".modal__point.modal__point--specialization").textContent = "Напрям тренера: " + foundElem.specialization;
			modal.querySelector(".modal__text").textContent = foundElem.description;
	...
	const sortBtns = document.querySelectorAll(".sorting__btn");
	...
	function sortingCards(btn) {
		if (btn == 0) {
			clearCards()
			trainersAppend(changedDATA)
		} else if (btn == 1) {
			let sortedDATA = changedDATA.slice().sort((a, b) => {
				const A = a["last name"].trim();
				const B = b["last name"].trim();
				return A.localeCompare(B);
			});
			clearCards()
			trainersAppend(sortedDATA)
		} else if (btn == 2) {
			let sortedDATA = changedDATA.slice().sort((a, b) => {
				const A = a.experience.replace(/[^\d-]/g, '');
				const B = b.experience.replace(/[^\d-]/g, '');
				return A.localeCompare(B);
			});
			clearCards()
			trainersAppend(sortedDATA)
	...
	const directionWrapper = document.querySelector(".filters__fieldset:nth-of-type(1)");
	const categoryWrapper = document.querySelector(".filters__fieldset:nth-of-type(2)");

	const directions = directionWrapper.querySelectorAll(".filters__input");
	const categories = categoryWrapper.querySelectorAll(".filters__input");

	const submitFiltersBtn = document.querySelector(".filters__submit")


	submitFiltersBtn.addEventListener("click", submitFilters, false);

	function checkDirection(dirId) {
		if (dirId == "all-direction") {
			changedDATA = DATA.slice();
		}	else if (dirId === "gym") {
			changedDATA = DATA.filter((trainer) => trainer.specialization == "Тренажерний зал")
		}	else if (dirId === "fight-club") {
			changedDATA = DATA.filter((trainer) => trainer.specialization == "Бійцівський клуб")
		}	else if (dirId === "kids-club") {
			changedDATA = DATA.filter((trainer) => trainer.specialization == "Дитячий клуб")
		}	else if (dirId === "swimming-pool") {
			changedDATA = DATA.filter((trainer) => trainer.specialization == "Басейн")
		};
		return changedDATA;
	};

	function checkCategory(catId) {
		if (catId == "all-category") {
			changedDATA = changedDATA.slice();
		}	else if (catId === "master") {
			changedDATA = changedDATA.filter((trainer) => trainer.category == "майстер")
		}	else if (catId === "specialist") {
			changedDATA = changedDATA.filter((trainer) => trainer.category == "спеціаліст")
		}	else if (catId === "instructor") {
			changedDATA = changedDATA.filter((trainer) => trainer.category == "інструктор")
		};

		return changedDATA;
	}

	function sumbitFiltersEvent() {
		clearCards();
		trainersAppend(changedDATA);
		sortingCards(curSortIndex);
	};

	function submitFilters(event) {
		const checkedDirectionId = Array.from(directions).find((checkbox) => checkbox.checked).getAttribute("id");
		const checkedCategoryId = Array.from(categories).find((checkbox) => checkbox.checked).getAttribute("id");
		
		checkDirection(checkedDirectionId);
		checkCategory(checkedCategoryId);
		sumbitFiltersEvent();
